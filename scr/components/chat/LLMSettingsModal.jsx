  ...
  useEffect(() => {
    ...
    base44.entities.LLMConfig.list().then((list) => {
      if (list.length > 0) {
        ...
        const savedToken = loadedConfig.api_key;
        const defaultToken = getDefaultHFToken();
        const finalToken = savedToken || defaultToken;
        setConfig({
            ...
            <>
              ...
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  placeholder={getDefaultHFToken() ? "Token configurado" : "Ingresa tu token hf_..."}
                  className="bg-input border-border text-sm"
                />
                <p className="text-[10px] text-muted-foreground">
                  Obtén tu token gratis en <a href="https://huggingface.co/settings/tokens" target="_blank" rel="noopener noreferrer" className="text-primary hover:underline">huggingface.co/settings/tokens</a>
                </p>
              </div>
            </>
          )}
