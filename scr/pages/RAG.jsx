...
const getDefaultHFToken = () => import.meta.env.VITE_HF_TOKEN || '';
...
export default function RAG() {
  ...
  const [hfToken, setHfToken] = useState(() => {
    ...
    return stored || getDefaultHFToken();
  });
  ...
  const getEmbedding = async (text) => {
    ...
    const token = hfToken || getDefaultHFToken();
  ...
  return (
    <div className="p-3 md:p-6 max-w-4xl mx-auto space-y-4 md:space-y-6">
      <h1 className="text-xl md:text-3xl font-bold text-foreground">Motor RAG: Indexación y Consulta</h1>

      {/* API Token Configuration */}
      <div className="space-y-3 p-3 md:p-4 border border-border rounded-lg bg-card">
        <h2 className="text-base md:text-lg font-semibold text-foreground">Configuración de API Token</h2>
      ...
      <div className="space-y-3 md:space-y-4 p-3 md:p-4 border border-border rounded-lg bg-card">
        <h2 className="text-base md:text-xl font-semibold text-foreground">Indexar Documento</h2>
        <input
          type="file"
          accept=".txt,.md,.docx"
          onChange={handleFileChange}
          className="block w-full text-xs md:text-sm text-foreground
            file:mr-2 md:file:mr-4 file:py-1.5 md:file:py-2 file:px-3 md:file:px-4
            file:rounded-md md:file:rounded-full file:border-0
      ...
      {/* Query Section */}
      <div className="space-y-3 md:space-y-4 p-3 md:p-4 border border-border rounded-lg bg-card">
        <h2 className="text-base md:text-xl font-semibold text-foreground">Consultar Fragmentos</h2>
        <input
          type="text"
          value={query}
          onChange={handleQueryChange}
          placeholder="Introduce tu consulta aquí..."
          className="block w-full p-2 md:p-2.5 border border-border rounded-md bg-background text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm"
