...
const getUserKey = (key) => `${key}_${getCurrentUserId()}`;
...
const createMockStorage = (key) => ({
  ...
  create: async (item) => {
    ...
    localStorage.setItem(getUserKey(key), JSON.stringify(list));
    return newItem;
  },
  ...
  delete: async (id) => {
    ...
    localStorage.setItem(getUserKey(key), JSON.stringify(filtered));
  },
