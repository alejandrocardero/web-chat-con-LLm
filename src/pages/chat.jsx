...
export default function Chat() {
  ...
  const sendMessage = async ({ text, attachment }) => {
    ...
    if (llmConfig.provider === 'huggingface') {
      // Hugging Face Inference API - OpenAI-compatible format
  ...
  return (
    <div className="flex h-[100dvh] bg-background overflow-hidden">
      ...
      {/* Sidebar */}
      <div className={`${sidebarOpen ? 'flex' : 'hidden'} md:flex flex-col h-full fixed md:relative z-40 md:z-auto w-72 md:w-auto transition-all`}>
        <ConversationSidebar
