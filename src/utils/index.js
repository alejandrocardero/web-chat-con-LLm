...
export default function Chat() {
  ...
  const loadLLMConfig = async () => {
    ...
    if (list.length > 0) setLlmConfig(list[0]);
  };
  ...
  const sendMessage = async ({ text, attachment }) => {
    if (!activeId) return;
    if (!llmConfig) { setSettingsOpen(true); return; }
    ...
    const apiUrl = llmConfig.provider === 'huggingface' 
      ? `${window.location.origin}/hf-api/chat/completions` 
    ...
    const assistantText = data.choices?.[0]?.message?.content || 'Sin respuesta del modelo.';
        ...
        <div className="flex-1 overflow-y-auto py-4 space-y-3">
          {!activeId && (
            <div className="flex flex-col items-center justify-center h-full gap-3 text-center px-8">
              ...
              <p className="text-foreground font-medium">Bienvenido a Chat con SLMs</p>
